...
import { FaApple, FaGoogle, FaMicrosoft, FaShoppingCart } from 'react-icons/fa'
...
const Navbar = () => {
    ...
    return (
        <>
            {!isSignUp && (
                <nav className={`flex items-center fixed z-40 top-0 left-0 w-full ${isVisible ? 'transform translate-y-0' : 'transform -translate-y-full'} ${isHeroSection ? 'bg-transparent' : '!bg-[#1b283f]'} transition-all duration-300`}>
                    <div className={`wrapper flex items-center justify-between ${!isHeroSection && '!py-3'} transition-all duration-300`}>
                        ...
                        <div className='md:flex items-center gap-6 w-full justify-end hidden'>
                            ...
                            <Button className={`border-2 text-gray-50 bg-transparent border-gray-50 font-medium rounded-[12px] hover:text-slate-800 hover:bg-gray-50 ${isDetails && isHeroSection && 'bg-blue-600'}`} onClick={() => setIsModalOpen(true)}>Login</Button>
                        ...
                        <div className={`absolute top-0 right-0 bg-gray-500 rounded-[12px] -translate-x-4 ${isOpen ? 'opacity-100 translate-y-[70px]' : 'opacity-100 -translate-y-full'} transition-all duration-300 ease-in-out`}>
                            <div className={`flex flex-col gap-2 p-5`}>
                                ...
                                <button className='font-medium text-[18px] text-gray-50 w-max' onClick={() => {
                                    setIsModalOpen(true)
                                    setIsOpen(false)
                                }}>Login</button>
                                <Link href="/auth/signup" className='font-medium text-[18px] text-gray-50' onClick={() => setIsOpen(false)}>
                                    Register
                                </Link>
                            </div>
                        </div>
