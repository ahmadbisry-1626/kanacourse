...
const TopCourseBusCard = ({ tb }: { tb: TbProps }) => {
    return (
        <Link href={`/business/${tb.id}/${encodeURIComponent(tb.title)}`} className='flex max-sm:flex-col gap-3 bg-gray-50 rounded-[12px] w-[620px] md:h-[350px] mt-6 hover:shadow-md transition-all duration-300 group overflow-hidden'>
            <div className='relative w-full md:max-w-[250px] h-full bg-[#1b283f] max-sm:rounded-t-[12px] md:rounded-tl-[12px] md:rounded-bl-[12px] overflow-hidden bg-transparent'>
                <Image src={tb.imgUrl} alt='' width={600} height={600} className='md:absolute rounded-tl-[12px] rounded-bl-[12px] w-full h-full object-cover transition-all duration-500' loading='lazy' />
                ...
                <div className='w-full p-2 rounded-t-[16px] absolute bottom-0 bg-[#1b283f] flex items-center justify-center transition-all duration-500 z-20'>
                    <span className='text-gray-50 font-semibold max-md:text-[14px]'>{tb.name}</span>
                </div>
            </div>

            <div className='flex flex-col justify-between gap-3 p-4 w-full'>
                <div className='flex flex-col gap-2'>
                    <h1 className='text-[18px] font-semibold'>{tb.title}</h1>
                    ...
                    <p className='text-[14px] text-gray-500'>{tb.description}</p>
                </div>
                ...
                <div className='flex flex-col gap-1'>
                    <div className='flex items-center gap-2'>
                        ...
                        <span className='max-md:text-sm'>({tb.rating})</span>
                    </div>
                    <div className='flex items-center gap-2 flex-wrap-reverse'>
                        <span className='text-[20px] font-semibold'>Rp. {tb.price.toLocaleString()}</span>
                        <span className='font-semibold line-through text-[16px] text-gray-500'>Rp. {tb.discount.toLocaleString()}</span>
                    </div>
                </div>
            </div>
        </Link>
...
export default TopCourseBusCard
