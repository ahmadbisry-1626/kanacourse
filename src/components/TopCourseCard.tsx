...
const TopCourseCard = ({ tc }: { tc: TcProps }) => {
    return (
        <Link href={`/design/${tc.id}/${encodeURIComponent(tc.title)}`} className='flex gap-3 bg-gray-50 rounded-[12px] w-[620px] md:h-[300px] mt-6 hover:shadow-md transition-all duration-300 group'>
            <div className='relative w-full md:max-w-[250px] max-w-[200px] h-full bg-[#1b283f] rounded-tl-[12px] rounded-bl-[12px] overflow-hidden'>
                <Image src={tc.imgUrl} alt='' width={300} height={300} className='absolute w-full h-full object-cover group-hover:translate-x-4 transition-all duration-500' sizes='100vw' loading='lazy'/>
